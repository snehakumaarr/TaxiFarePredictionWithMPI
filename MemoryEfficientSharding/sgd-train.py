...
def compute_rmse_mpi(model, start, end, data_directory, mode, comm):
    ...
    with torch.no_grad():
        ...
        for idx in range(start, end):
            if mode == "train":
                X_path = f"{data_directory}/X_train/trainX_batch_{idx}.npy"
                y_path = f"{data_directory}/y_train/trainy_batch_{idx}.npy"
            elif mode == "val":
                X_path = f"{data_directory}/X_valid/valX_batch_{idx}.npy"
                y_path = f"{data_directory}/y_valid/valy_batch_{idx}.npy"
            else:
                X_path = f"{data_directory}/X_test/testX_batch_{idx}.npy"
                y_path = f"{data_directory}/y_test/testy_batch_{idx}.npy"  
            X_sample = torch.from_numpy(np.load(X_path)).float().to(DEVICE)
            y_sample = torch.from_numpy(np.load(y_path)).float().to(DEVICE)
            y_sample = y_sample.squeeze(-1) # From (2048, 1) -> (2048)
            y_pred = model(X_sample)
            batch_sum_sq = float(((y_pred - y_sample) ** 2).sum().item())
            batch_n = int(X_sample.shape[0])
            local_sum_sq += batch_sum_sq
            local_n += batch_n
...
def train(comm, args):
    ...
    # Sizes of dataset
    train_size = len(os.listdir(f'{args.data_directory}/X_train')) - 1 # Minus 1 to remove the last guy that doesn't have 2048 samples
    test_size = len(os.listdir(f'{args.data_directory}/X_test'))

    print("Loading Datasets!")
    if validation_mode:
        val_size = len(os.listdir(f'{args.data_directory}/X_valid'))
        dataset = Dataset(args.data_directory, comm, train_size, test_size, args.split_dataset_size, validation_mode = validation_mode, val_size=val_size)
    else:
        dataset = Dataset(args.data_directory, comm, train_size, test_size, args.split_dataset_size)
    m_features = dataset.m_features
    ...
    for epoch in range(args.epochs):
        ...
        # Convert numpy arrays to torch tensors, cast to float32, then move to device
        ...
        if epoch % args.log_every == 0:
            train_rmse = compute_rmse_mpi(model, dataset.train_start, dataset.train_end, args.data_directory, "train", comm)
            if validation_mode:
                val_rmse = compute_rmse_mpi(model, dataset.val_start, dataset.val_end, args.data_directory, "val", comm)
    ...
    final_test_rmse = compute_rmse_mpi(model, dataset.test_start, dataset.test_end, args.data_directory, "test", comm)
    ...
    if rank == 0:
        ...
        filename = f"results/rmse_results_memeff_np{size}_lr{args.learning_rate}_bs{args.batch_size}_nh{fn_hidden}_{args.activation}_ep{args.epochs}"
        if validation_mode:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--data_directory", type=str, default="../Data_Split")
